...
const replaceZeros = (string) => {
  ...
  let replaceZero =string.replace(regexp,'zero');
  return replaceZero;
}
...
const addOne = (arr) => {
  ...
  let storedNewValues =[];
  arr.map(element =>{
    storedNewValues.push(element+1); 
  })
  return storedNewValues;
}
...
const forLoopTwoToThe = (arr) => {
  ...
  for(let i =0; i <arr.length;i++){
    let powerTo = Math.pow(2,arr[i]);
    powerArray.push(powerTo)
  }
  return powerArray;
};
