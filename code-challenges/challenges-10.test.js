...
const count = (target, input) => {
  ...
  return input.reduce((acc,value)=>{
    acc = acc + value.reduce((acc , values )=>{
      if (values === target){
      acc = acc+1
    }
    return acc;
...
const totalSum = (input) => {
  // Solution code here...
  return input.reduce((acc,value)=>{
    acc = acc + value.reduce((acc , elements)=>{
     acc = acc+elements; 
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let arrayFilter = input.map(element =>element.filter (value => typeof(value) === 'number' && !(value % 5)));
  let results = arrayFilter.map(item => item.map(element => Math.pow(2,element)));
  return results;
};
...
let findMaleAndFemale = (data) => {
  ...
  let array = data.filter(value =>{
    if (value.gender ==='female'|| value.gender ==='male'){ return value.name;} 
...
let findShortest = (data) => {
  // Solution code here...
  const shortest = data.reduce((shortesAcc,value)=>{
    return (Number (shortesAcc.height) < Number (value.height))?shortesAcc:value;
  })
